...
async def show_channel(ctx, arg1=None): #show channel command
  ...
  print(f"Şuna dak bak {channel_lists} bir de buna {command_channel_ids}")
  ...
  if payload.emoji.name in emoji_lists: #checking the reaction emoji is our list or not
    ...
    for reaction in message.reactions:
      if reaction.emoji == payload.emoji.name: #checking the emoji is in our list or not
        ...
        if EmojiCount == int(channel_lists[index1][2]): #checking EmojiCount is equal our count or not

          if int(payload.channel_id)!= int(channel_lists[index1][0]): #checking channel id is equal our channel id or not
            return
